...
class Poss {
  constructor(baseParams) {
    if (baseParams.debug) {
      console.log('[poss-sdk] sdk is in debug mode')
      logger.transports[0].silent = false
    } else {
      logger.transports[0].silent = true
    }
    if (!baseParams || !baseParams.possExecutablePath) {
      logger.info('You did not provide the path of poss executable!')
    }
    this.possPath = baseParams.possExecutablePath
    logger.info(`Poss created! Poss executable: ${this.possPath}`)
  ...
  setPossExecutablePath(path) {
    logger.info(`setting poss executable path: ${path}`)
    this.possPath = path
  }
  ...
  setDefaultBucket(bucketName) {
    ...
    // return this.callMethod('CreateBucket', { bucket: bucketName })
    //   .then(() => {
    //     this.baseParams.bucket = bucketName
    //     return bucketName
    //   })
    //   .catch(err => {
    //     logger.error('set default bucket failed!')
    //     throw err
    //   })
  }

  callMethod(method, params) {
    logger.info(`Calling POSS method ${method}, with params: `)
  ...
  startDaemon(params) {
    ...
    const fs = require('fs')
    if (this.runningDaemonPort !== 0) {
      logger.warn(
        `There is already a daemon running for this poss instance on port: ${
      ...
      .then(() => {
        return new Promise((resolve, reject) => {
          ...
          execCmd(
            ...
            exitCode => {
              logger.error(`Poss daemon exited with code ${exitCode}`)
              delete this.baseParams.rpcport
              this.runningDaemonPort = 0
  ...
  initDaemon(params) {
    ...
    return new Promise((resolve, reject) => {
      ...
      execCmd(this.possPath, 'init', initConfig, undefined, undefined, exitCode => {
